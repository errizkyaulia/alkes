...
<script>
    ...
    function nextStep(step) {
        ...
        if(step === 3) {
            ...
            // gunakan format ul li
            let data = "<ul>";
            for(let pair of formData.entries()) {
                data += `<li>${pair[0]}: ${pair[1]}</li>`;
            }
            data += "</ul>";
...
<script>
    ...
    fetch("https://www.emsifa.com/api-wilayah-indonesia/api/provinces.json")
        ...
        .then(data => {
            ...
            data.forEach(prov => {
                ...
                option.value = prov.name; // Menggunakan nama sebagai value
                option.textContent = prov.name;
                option.setAttribute("data-id", prov.id); // Simpan ID dalam atribut data-id
    ...
    document.getElementById("provinsi").addEventListener("change", function() {
        ...
        let provinsiId = selectedOption.getAttribute("data-id"); // Ambil ID dari atribut data-id
